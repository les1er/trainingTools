...
local update_path = getWorkingDirectory() .. "/Training Tools/update.ini"
	...
	downloadUrlToFile(update_url, update_path, function(id, status)
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
			...
			if tonumber(updateIni.info.vers) > script_vers then
				sampAddChatMessage(tag .. "{FFFFFF}Доступна новая версия скрипта:{80BCFF} " .. updateIni.info.vers_text, 0xFFFFFFFF)
				update_state = true
			end
	...
	while true do
		...
		if update_status then
			downloadUrlToFile(script_url, script_path, function(id, status)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					sampAddChatMessage(tag .. "{FFFFFF}Скрипт {80BCFF}успешно {FFFFFF}обновлен.", 0xFFFFA500)
					thisScript():reload()
				end
			end)
			break
...
function autopame_cmd(arg)
	...
	sampSendDialogResponse(32700, 1, -1, apame_line1.v)
	--2 stroka
	sampSendDialogResponse(32700, -1, 1, apame_line2.v)
	sampSendDialogResponse(32700, 1, -1, apame_line2.v)
	--3 stroka
	sampSendDialogResponse(32700, -1, 2, apame_line3.v)
	sampSendDialogResponse(32700, 1, -1, apame_line3.v)
	--4 stroka
	sampSendDialogResponse(32700, -1, 3, apame_line4.v)
	sampSendDialogResponse(32700, 1, -1, apame_line4.v)
	--5 stroka
	sampSendDialogResponse(32700, -1, 4, apame_line5.v)
	sampSendDialogResponse(32700, 1, -1, apame_line5.v)
	--6 stroka
	sampSendDialogResponse(32700, -1, 5, apame_line6.v)
	sampSendDialogResponse(32700, 1, -1, apame_line6.v)
	--7 stroka
	sampSendDialogResponse(32700, -1, 6, apame_line7.v)
	sampSendDialogResponse(32700, 1, -1, apame_line7.v)
	--8 stroka
	sampSendDialogResponse(32700, -1, 7, apame_line8.v)
	sampSendDialogResponse(32700, 1, -1, apame_line8.v)
	--9 stroka
	sampSendDialogResponse(32700, -1, 8, apame_line9.v)
	sampSendDialogResponse(32700, 1, -1, apame_line9.v)
	--10 stroka
	sampSendDialogResponse(32700, -1, 9, apame_line10.v)
	sampSendDialogResponse(32700, 1, -1, apame_line10.v)
end
